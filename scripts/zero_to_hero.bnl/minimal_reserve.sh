...
if [[ -f "$INSTANCE_URI_FILE" ]]; then
    ...
    if apptainer exec --env EJFAT_URI="$EJFAT_URI" --net "${E2SAR_IMAGE:-docker://ibaldin/e2sar:0.3.1a3}" lbadm --overview &>/dev/null; then
...
apptainer exec --env EJFAT_URI="$EJFAT_URI" "$E2SAR_IMAGE" lbadm --reserve --lbname "$LB_NAME" --export > "$INSTANCE_URI_FILE"
