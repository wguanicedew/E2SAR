...
if [[ -n "${INSTANCE_URI:-}" ]]; then
    # INSTANCE_URI is an environment variable
    EJFAT_URI="$INSTANCE_URI"
    echo "Using EJFAT_URI from INSTANCE_URI environment variable"

elif [[ -f "$INSTANCE_URI_FILE" ]]; then
...
CMD=(
    apptainer
    exec
    --env "EJFAT_URI=$EJFAT_URI"
    --env "MALLOC_ARENA_MAX=32"
    "$E2SAR_IMAGE"
    e2sar_perf
    --send
    --withcp
